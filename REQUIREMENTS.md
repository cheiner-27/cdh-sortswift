...
**Merge Duplicate Inventory Rows:** A utility scans filtered active inventory for matching rows — same card identity + condition + printing + language **and** same bin, comment, price overrides/floors, and marketplace listing links/settings — and merges their quantities into the oldest row, soft-deleting the extras. Language codes compare without case or surrounding spaces (`EN` = `en`), and intake uses the same comparison to avoid creating another duplicate. The surviving row retains its current price. FIFO purchase batches retain their separate costs, dates and balances; different purchase costs within the shared FIFO pool do not prevent merging. Redundant listing links on retired rows are removed so learned SKUs keep resolving to the surviving row. Rows differing in meaningful identity or settings (e.g. two bins or languages) stay separate. Irreversible.
